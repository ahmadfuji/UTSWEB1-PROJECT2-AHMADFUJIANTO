...
if (!empty($_POST['id'])){
    try{
        move_upload_file($photo_tmp, '../photo/' . $photo_name);
        ...
        $connect->execute([$nama_depan, $nama_belakang, $email, sha1($password), 'photo/' . $photo_name, $_POST['id']]);
    ...
    try {
        ...
        $connect = $database_connection-prepare($sql);
        $connect->execute([$nama_depan, $nama_belakang, $email, sha1($password), 'photo/' . $photo_name]);
